...
export const objectIntakeSourceDefinitions = {
  ...
  buyer_upload: {
    ...
    source_note: "Professional global buyer intake for new-goods commerce.",
    reward_eligible: false,
  ...
  windkeep_member: {
    ...
    source_note: "Future member-center secondhand continuity entry.",
...
function cloneStore(value) {
  return {
    ...
    airEngineJobs: Array.isArray(value?.airEngineJobs) ? value.airEngineJobs : [],
...
export function makeProductDraft(input = {}) {
  ...
  return {
    ...
    draft_description: input.original_description || `${title} is prepared as a clear Dohara object draft with room, material, and use kept plain.`,
    material: "Material to confirm after human review.",
    size_text: "Size to confirm after measuring.",
    category,
    tags,
    seo_title: `${title} - Dohara`,
    seo_description: `${title} prepared through the Dohara object intake pipeline with quiet material presence.`,
    geo_summary: `${title} is a Dohara object candidate with source, media, review, and publication state attached to one object_id.`,
