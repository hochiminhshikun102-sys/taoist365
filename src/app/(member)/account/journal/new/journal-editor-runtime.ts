...
export function bindJournalEditor(root){
 ...
 q('[data-control-id="cover-file"]')?.addEventListener('change',e=>{const f=e.target.files?.[0];if(!f)return;if(!['image/jpeg','image/png'].includes(f.type)||f.size>10*1024*1024){e.target.value='';return toast('Use JPG or PNG up to 10 MB.')}toast('Cover selected for this draft.')});
 q('[data-action="remove-cover"]')?.addEventListener('click',()=>{const f=q('[data-control-id="cover-file"]');if(f)f.value='';toast('Cover removed from this draft.')});
