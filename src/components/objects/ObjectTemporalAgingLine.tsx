...
export function ObjectTemporalAgingLine({ catalogPieceId }: Props) {
  const { presence, aging, inertia, lowSignalHumanity } = useWorldRuntime();
  ...
  return (
    <div className="mt-2 space-y-1.5">
      <p className="text-[0.62rem] leading-5 text-text-muted/46 italic">
        Time passing through materials · {line} World age: {aging.ageLabel}. Object language: {inertia.objectLanguageThinning} /{" "}
        {lowSignalHumanity.objectFamiliarityLine}
      </p>
      <p className="text-[0.6rem] leading-[1.55] text-text-muted/42">Touch sediment · {touch}</p>
    </div>
  );
}
