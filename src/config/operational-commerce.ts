...
const objectDetails = {
  ...
  "taoist365-night-teacup": {
    ...
    subtitle: "Ceramic mug with small Dohara graphic.",
...
function autoMaterialSeed(stem: string, index: number, fromSplit = false): MaterialObjectSeed {
  ...
  return {
    ...
    sourceStem: stem,
    title,
    subtitle: isWindBell ? "Decorative room wind bell prepared from the current product shelf." : fromSplit ? "Cropped product candidate from a multi-object source image." : "First-stage product link from the current Dohara material shelf.",
    ...
    inspiration: isWindBell
      ...
      : "This link keeps the product shape from the original material while resetting the surrounding field into a softer Dohara surface.",
    collection,
...
const foundationalCommerceObjects: readonly CommerceObject[] = taoist365ObjectsCatalog.map((piece, index) => {
  ...
  return {
    ...
    trustNotes: [
      ...
      "Photos are current material references from the Dohara asset shelf.",
    ...
    media: {
      ...
      alt: `${details.title} photographed for Dohara`,
      caption: `${details.title} in a real product surface from the Dohara material shelf.`,
...
const materialProductObjects: readonly CommerceObject[] = allMaterialObjectSeeds.map((seed, index) => {
  ...
  return {
    ...
    media: {
      ...
      caption: `${seed.title} staged from the current Dohara product material shelf.`,
